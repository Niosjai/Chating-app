...
def handle_join_room_event(data):
    ...
    # Broadcast join announcement and active user count
    emit("join_announcement", {
        "message": f"{name} has joined the room!",
        "active_count": len(active_rooms[chat_room_id])
    }, to=chat_room_id)
...
def handle_disconnect():
    ...
    if user:
        ...
        # Broadcast leave announcement and active user count
        emit("leave_announcement", {
            "message": f"{name} has left the room.",
            "active_count": len(active_rooms.get(chat_room_id, []))
        }, to=chat_room_id)

@app.route("/active_users1")
def get_active_users1():
    chat_room_id = request.args.get("chat_room_id")
    if chat_room_id in active_rooms:
        return jsonify({"users": list(active_rooms[chat_room_id])})
...
def get_active_users():
    chat_room_id = request.args.get("chat_room_id")
    if chat_room_id and chat_room_id in active_rooms:
        users = list(active_rooms[chat_room_id])
